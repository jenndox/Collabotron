...
var sliders = [ "popularity", "nearby", "soon", "colors"];
...
function onLoad() {
  ...
  if(mainCanvas.getContext)
  {
    ...
    viewport.sceneBounds = cloud.frame;
...
function renderTags() {
    ...
    cloud.search(ctx, tagName, sliders[xFactor], sliders[yFactor], sliders[sizeFactor], showBubbles);
...
function setupTagInput() {
    ...
    handlers.onMouseUp = function (button, x, y, el) {
        ...
        if (mouseDelta < 4) {
            if (shiftDown == true) {
                ...
                var sxy = viewport.screenToScene(x, y);
                cloud.findClosest(mainCanvas.getContext("2d"), sxy[0], sxy[1], viewport.cameraScale.current);
                mpmetrics.track("Pan the triangle");
    ...
    handlers.onMouseWheel = function (delta, x, y, el) {
        if (shiftDown == true) {
            ...
            viewport.stopPan();
            viewport.zoomToScaleKeepingViewPointConstant(newScale, x - 80, y - 250, 1.0);
            mpmetrics.track("Mousewheel the triangle");
        }
    }
    ...
    handlers.onTouchEnd = function (button, x, y, el) {
        ...
        mouseDelta = 0;
        mpmetrics.track("Touch the triangle");
    }
...
function searchTags() {
    ...
    mode = "text";
    cloud.search(ctx, tagName, sliders[xFactor], sliders[yFactor], sliders[sizeFactor], showBubbles);
    mpmetrics.track("Searched for tag name");
}
...
function searchByColor() {
    ...
    ctx.font = "30px georgia";
    mode = "colors";
    cloud.searchColor(ctx, sliders[sizeFactor], showBubbles, cHigh, cLow, mHigh, mLow, yHigh, yLow 
    //mpmetrics.track('Search by color', { 'cHigh' : cHigh , 'cLow' : cLow , 'mHigh' : mHigh , 'mLow' : mLow, 'yHigh' : yHigh , 'yLow' : yLow } );
);
...
function updateTagSearch(newValue) {
    tagName = newValue;
