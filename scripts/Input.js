function setupInput(displayRoot, handlers) {
    ...
    displayRoot.ongesturestart = function (event) {
        if (handlers.onGestureStart) {
            var button = 0;
             // Assume we only care about the first two touches.
            var x1 = event.touches[0].clientX - event.target.clientLeft;
            var y1 = event.touches[0].clientY - event.target.clientTop;
            var x2 = event.touches[1].clientX - event.target.clientLeft;
            var y2 = event.touches[1].clientY - event.target.clientTop;
            handlers.onGestureStart(button, x1, y1, x2, y2, event.target);
        }
        if (event.preventDefault) {
            event.preventDefault();
        }
    ...
    displayRoot.ongestureend = function (event) {
        if (handlers.onGestureEnd) {
            ...
            var x2 = event.touches[1].clientX - event.target.clientLeft;
            var y2 = event.touches[1].clientY - event.target.clientTop;
            handlers.onGestureEnd(button, x1, y1, x2, y2, event.target);
        }
        if (event.preventDefault) {
            event.preventDefault();
        }
        ...
        if (event.scale) {
            if (handlers.onGestureChange) {
                ...
                if (!isNaN(x1) && !isNaN(y1) && !isNaN(x2) && !isNaN(y2)) {
                    handlers.onGestureChange(event.scale, x1, y1, x2, y2, event.target);
                }
            }
